...
case "fields":
	...
	$sql = "SELECT * FROM ".$xoopsDB->prefix('xml_tables')." where view = '0'";
	$ret = $xoopsDB->queryF($sql);
	
	$form_sel = new XoopsThemeForm(_XXMLC_SELECTTABLE, "seltable", $_SERVER['PHP_SELF'] ."");
	$form_sel->setExtra( "enctype='multipart/form-data'" ) ;
	
	$table_sel = new XoopsFormSelect(_XXMLC_SELECTTABLE.':', 'select');
	...
	$form_fld = new XoopsThemeForm(_XXMLC_FIELDOPTIONSFOR.' '.get_tablename($tbl_id), "fields", $_SERVER['PHP_SELF'] ."");
...
case "savefields":
	...
	foreach ($id as $f){
		...
		switch ($f){
		case "new":
			$sql = "INSERT INTO ".$xoopsDB->prefix('xml_fields')." (tbl_id, fieldname, allowpost, allowretrieve, allowupdate, visible, `key`, `string`, `int`, `float`, `text`, `other`, `crc`) VALUES ('$tbl_id','".addslashes($fieldname[$tt])."','".intval($post[$tt])."','".intval($retrieve[$tt])."','".intval($update[$tt])."','".intval($visible[$tt])."','".intval($key[$tt])."','".intval($string[$tt])."','".intval($int[$tt])."','".intval($float[$tt])."','".intval($text[$tt])."','".intval($other[$tt])."','".intval($crc[$tt])."')";
			$ty=$xoopsDB->queryF($sql);
			break;
		default:
			$sql = "UPDATE ".$xoopsDB->prefix('xml_fields')." SET allowpost ='".intval($post[$tt])."', allowupdate ='".intval($update[$tt])."',allowretrieve = '".intval($retrieve[$tt])."', visible='".intval($visible[$tt])."',`key` ='".intval($key[$tt])."', `string` = '".intval($string[$tt])."', `int`='".intval($int[$tt])."',`float` ='".intval($float[$tt])."', `text` = '".intval($text[$tt])."', `other`='".intval($other[$tt])."', crc = '".intval($crc[$tt])."' WHERE fld_id = ".$id[$tt]. " and tbl_id = ".$tbl_id;
			$ty=$xoopsDB->queryF($sql);
		}
	
	}
	redirect_header("index.php?op=fields&tbl_id=".$tbl_id,2,_XXMLC_DATABASEUPDATED);
	break;
...
case "savetables":
	...
	foreach ($id as $f){
		...
		switch ($f){
		case "new":
			$sql = "INSERT INTO ".$xoopsDB->prefix('xml_tables')." (tablename, allowpost, allowretrieve, allowupdate, visible, view) VALUES ('".addslashes($tablename[$tt])."','".intval($post[$tt])."','".intval($retrieve[$tt])."','".intval($update[$tt])."','".intval($visible[$tt])."','0')";
			$ty=$xoopsDB->queryF($sql);
			break;
		default:
			$sql = "UPDATE ".$xoopsDB->prefix('xml_tables')." SET allowpost ='".intval($post[$tt])."', allowretrieve = '".intval($retrieve[$tt])."', allowupdate = '".intval($update[$tt])."', visible='".intval($visible[$tt])."' WHERE tbl_id = ".$id[$tt];
			$ty=$xoopsDB->queryF($sql);
		}
	
	}
	redirect_header("index.php?op=tables",2,_XXMLC_DATABASEUPDATED);
	break;
...
case "saveviews":
	...
	foreach ($id as $f){
		...
		switch ($f){
		case "new":
			$sql = "INSERT INTO ".$xoopsDB->prefix('xml_tables')." (tablename, allowpost, allowretrieve, allowupdate, visible, view) VALUES ('".addslashes($tablename[$tt])."','".intval($post[$tt])."','".intval($retrieve[$tt])."','".intval($update[$tt])."','".intval($visible[$tt])."','1')";
			$ty=$xoopsDB->queryF($sql);
			break;
		default:
			$sql = "UPDATE ".$xoopsDB->prefix('xml_tables')." SET allowpost ='".intval($post[$tt])."', allowretrieve = '".intval($retrieve[$tt])."', allowupdate = '".intval($update[$tt])."', visible='".intval($visible[$tt])."' WHERE tbl_id = ".$id[$tt];
	...
	redirect_header("index.php?op=views",2,_XXMLC_DATABASEUPDATED);
	break;
...
case "views":
	...
	$ele_tray = array();
	$form_view = new XoopsThemeForm(_XXMLC_VIEWSFOR.' '.XOOPS_DB_NAME, "views", $_SERVER['PHP_SELF'] ."");
...
case "saveplugins":
	...
	foreach ($id as $f){
		...
		switch ($f){
		case "new":
			$sql = "INSERT INTO ".$xoopsDB->prefix('xml_plugins')." (plugin_name, plugin_file, active) VALUES ('".addslashes($functionname[$tt])."','".addslashes($filename[$tt])."','".intval($active[$tt])."')";
			$ty=$xoopsDB->queryF($sql);
			break;
		default:
			$sql = "UPDATE ".$xoopsDB->prefix('xml_plugins')." SET active ='".intval($active[$tt])."' WHERE plugin_id = ".$id[$tt];
			$ty=$xoopsDB->queryF($sql);
		}
	
	}
	redirect_header("index.php?op=plugins",2, _XXMLC_SAVEDSUCCESSFUL);
	break;
...
case "plugins":
	...
	foreach($funct->GetServerExtensions() as $extension){
		$phpcode= file_get_contents(XOOPS_ROOT_PATH.'/modules/xxml/plugins/'.$extension);
		ob_start();
		$r=eval("?>".$phpcode."<?php
");
	...
	$ele_tray = array();
	$form_plugin = new XoopsThemeForm(_XXMLC_PLUGINAVAILABLE, "plugins", $_SERVER['PHP_SELF'] ."");
...
default:
	...
	$ele_tray = array();
	$form_tables = new XoopsThemeForm(_XXMLC_TABLESAVAILABLE.' '.XOOPS_DB_NAME, "tables", $_SERVER['PHP_SELF'] ."");
